...
let state=null,reqSeq=0,pending=false,needRender=false,scale=1,dpr=1,anim=null,animKey='',pollTimer=0,lastStateStamp='',actionQueue=Promise.resolve(),lastAppliedSeq=0,lastPerfPostAt=0,lastTacticSeq=0,tacticOverlayText='',tacticOverlayUntil=0,courtBannerTimer=0,lastOpeningSeq=0,lastEndgamePresentationKey='',gameTypeIntent='';
...
function syncSoundToggle(){const btn=document.getElementById('soundToggle');if(!btn)return;const strong=btn.querySelector('strong');const span=btn.querySelector('span');if(strong)strong.textContent='音效';if(span)span.textContent=soundEnabled?'当前开启':'当前关闭';btn.setAttribute('aria-pressed',soundEnabled?'true':'false');}function toggleSound(){soundEnabled=!soundEnabled;localStorage.setItem('xq_sound_enabled',soundEnabled?'1':'0');syncSoundToggle();}function unlockAudio(){if(audioUnlocked)return;audioUnlocked=true;[moveAudio,mateAudio].forEach(a=>{const p=a.play();if(p&&p.catch){p.then(()=>{a.pause();a.currentTime=0;}).catch(()=>{});}else{a.pause();a.currentTime=0;}});}function playSound(a){if(!soundEnabled||!audioUnlocked)return;try{a.pause();a.currentTime=0;const p=a.play();if(p&&p.catch)p.catch(()=>{});}catch(_e){}}
...
if(endgameReplay)endgameReplay.addEventListener('click',()=>document.getElementById('newGame').click());
if(endgameReview)endgameReview.addEventListener('click',()=>document.getElementById('reviewStart').click());
...
const THEMES={minimal:'米杏',ink:'墨青',imperial:'鎏金'};
...
function updateSetupSummary(){
  ...
  const theme=THEMES[currentTheme()]||'米杏';
  ...
  if(themeNote)themeNote.textContent='当前主题：'+theme+' · 可在设置中切换';
}
...
function updateLandingStatus(){
  ...
  landingStatus.textContent='系统状态：待开始，可直接进入 AI 对局，也可前往在线大厅';
}
...
function syncHomeChrome(){if(ui.mode){ui.mode.value='pvc';ui.mode.disabled=true;}setDis(ui.undo,true);setDis(ui.drawBtn,true);if(endgameTag&&endgameTag.textContent.indexOf('残局')===0)endgameTag.textContent='当前模式：AI 对局';const game=(ui.gameType&&ui.gameType.value)===GAME_GOMOKU?'五子棋':'中国象棋';const diff=selectedText(document.getElementById('difficulty'))||'中等';if(summary)summary.textContent=[game,'AI 对局',diff,'米杏'].join(' · ');if(leadTitle)leadTitle.textContent=state&&state.started?'AI 对局进行中，可继续落子、复盘或返回首页':(currentUser?'棋桌已就绪，点击“开始 AI 对局”即可开局':'先登录，再开始 AI 对局');if(ui.info&&!state?.started)ui.info.textContent=currentUser?'选择棋种、难度和先后手后开始 AI 对局':'请先登录，再开始 AI 对局';if(landingStatus&&!state?.started)landingStatus.textContent=currentUser?'系统状态：已登录，可开始 AI 对局或前往在线大厅':'系统状态：未登录，可先登录或直接前往在线大厅';if(modeLabel&&modeLabel.textContent.includes('模式:'))modeLabel.textContent=modeLabel.textContent.replace('模式: 人机','模式: AI').replace('模式: 双人','模式: AI');if(startButton){const strong=startButton.querySelector('strong');const span=startButton.querySelector('span');if(strong)strong.textContent=state&&state.started?'重新开始 AI 对局':'开始 AI 对局';if(span)span.textContent=currentUser?'按当前设置立即开局':'登录后即可开局';}}
