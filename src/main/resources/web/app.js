...
let state=null,reqSeq=0,pending=false,needRender=false,scale=1,dpr=1,anim=null,animKey='',pollTimer=0,lastStateStamp='',actionQueue=Promise.resolve(),lastAppliedSeq=0,lastPerfPostAt=0,lastTacticSeq=0,tacticOverlayText='',tacticOverlayUntil=0,courtBannerTimer=0,lastOpeningSeq=0,gameTypeIntent='',actionBusy=false,busyHint='';
...
async function act(path){try{const data=await api(path);busyHint='';applyState(data);}catch(err){busyHint='';showUiError(err);throw err;}finally{actionBusy=false;if(state)applyState(state);}}function enqueueAct(path,busyText){actionQueue=actionQueue.then(()=>{actionBusy=true;busyHint=busyText||'';if(state)applyState(state);return act(path);}).catch(()=>{});return actionQueue;}
function cloneBoardState(board){
  return Array.isArray(board) ? board.map(row => Array.isArray(row) ? row.map(cell => cell ? { ...cell } : null) : []) : [];
}
function applyOptimisticPracticeMove(fromRow, fromCol, toRow, toCol){
  if(!state||state.gameType!==GAME_XIANGQI||state.mode!=='PVC'||!Array.isArray(state.board))return;
  const board=cloneBoardState(state.board);
  const moving=board[fromRow]&&board[fromRow][fromCol]?{...board[fromRow][fromCol]}:null;
  if(!moving)return;
  if(!board[toRow])board[toRow]=[];
  board[fromRow][fromCol]=null;
  board[toRow][toCol]=moving;
  const recent=[{order:1,color:moving.color,fromRow,toCol,toRow,fromCol}];
  if(Array.isArray(state.recentMoves)&&state.recentMoves.length){
    const prev=state.recentMoves[0];
    if(prev)recent.push({...prev,order:2});
  }
  const nextState={...state,board,recentMoves:recent,selectedRow:-1,selectedCol:-1,currentTurn:state.pvcHumanColor==='RED'?'BLACK':'RED'};
  applyState(nextState);
}
document.addEventListener('pointerdown',unlockAudio,{once:true});function onBoardPointer(e,el){if(actionBusy||state&&(!state.started||state.reviewMode||state.gameOver))return;e.preventDefault();const rect=el.getBoundingClientRect();const sx=BASE_W/rect.width,sy=BASE_H/rect.height;const x=(e.clientX-rect.left)*sx,y=(e.clientY-rect.top)*sy;const g=pickGrid(x,y);if(!g)return;const hadSelection=!!(state&&state.selectedRow>=0&&state.selectedCol>=0);const fromRow=state?state.selectedRow:-1;const fromCol=state?state.selectedCol:-1;const p=state&&state.board&&state.board[g.row]?state.board[g.row][g.col]:null;const isReselect=hadSelection&&p&&p.color===state.currentTurn&&(state.mode!=='PVC'||p.color===state.pvcHumanColor);if(state&&state.selectedRow<0&&p&&p.color===state.currentTurn&&(state.mode!=='PVC'||p.color===state.pvcHumanColor)){state.selectedRow=g.row;state.selectedCol=g.col;scheduleRender();}if(hadSelection&&!isReselect&&state&&state.mode==='PVC'&&state.gameType===GAME_XIANGQI){applyOptimisticPracticeMove(fromRow,fromCol,g.row,g.col);}const busyText=hadSelection&&state&&state.mode==='PVC'&&!isReselect?'AI 思考中...':'处理中...';enqueueAct('/api/click?row='+g.row+'&col='+g.col,busyText);}canvas.addEventListener('pointerdown',e=>onBoardPointer(e,canvas),{passive:false});gomokuCanvas.addEventListener('pointerdown',e=>onBoardPointer(e,gomokuCanvas),{passive:false});
