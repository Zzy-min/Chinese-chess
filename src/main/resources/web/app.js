...
document.addEventListener('pointerdown',unlockAudio,{once:true});function onBoardPointer(e,el){if(actionBusy||state&&(!state.started||state.reviewMode||state.gameOver))return;e.preventDefault();const rect=el.getBoundingClientRect();const sx=BASE_W/rect.width,sy=BASE_H/rect.height;const x=(e.clientX-rect.left)*sx,y=(e.clientY-rect.top)*sy;const g=pickGrid(x,y);if(!g)return;const hadSelection=!!(state&&state.selectedRow>=0&&state.selectedCol>=0);const p=state&&state.board&&state.board[g.row]?state.board[g.row][g.col]:null;if(state&&state.selectedRow<0&&p&&p.color===state.currentTurn&&(state.mode!=='PVC'||p.color===state.pvcHumanColor)){state.selectedRow=g.row;state.selectedCol=g.col;scheduleRender();}const busyText=hadSelection&&state&&state.mode==='PVC'?'AI 思考中...':'处理中...';enqueueAct('/api/click?row='+g.row+'&col='+g.col,busyText);}canvas.addEventListener('pointerdown',e=>onBoardPointer(e,canvas),{passive:false});gomokuCanvas.addEventListener('pointerdown',e=>onBoardPointer(e,gomokuCanvas),{passive:false});
