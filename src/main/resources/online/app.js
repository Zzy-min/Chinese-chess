...
const ME_SUB_ROUTES = ['overview', 'records', 'study', 'inbox', 'achievements', 'settings', 'help'];
...
async function loadMe() {
  ...
  if (!state.me) {
    ...
    state.profilePreferences = null;
  }
}

async function loadProfilePreferences(renderAfter = false) {
  if (!state.me) {
    state.profilePreferences = null;
    state.boardTheme = 'wood';
  ...
  const preferences = await fetchJson(`${API_BASE}/profile/preferences`).catch(() => null);
...
async function loadCommunityLeaderboard() {
  state.communityLeaderboard = await fetchJson(`${API_BASE}/community/leaderboard`).catch(() => ({ winBoard: [], activityBoard: [], byGameType: {} }));
  render();
}
...
async function loadLobbySearch(query, renderAfter = true) {
  ...
  const result = await fetchJson(`${API_BASE}/lobby/search?q=${encodeURIComponent(normalized)}&limit=8`).catch(error => ({
    query: normalized,
    rooms: [],
    players: [],
    error: error.message || '搜索失败'
  }));
  if (requestId !== state.lobbySearchRequestId) {
    return;
  }
  state.lobbySearch = {
    query: normalized,
...
function currentRoute() {
  ...
  const meTab = page === 'me' ? resolveMeSubRoute(id || 'overview') : '';
  const puzzleTheme = page === 'learn' && learnTab === 'puzzles'
    ? resolvePuzzleTheme(parts[2] || 'ALL')
    : resolvePuzzleTheme(state.learnPuzzleTheme);
  return { page: page, id: id, leaf: parts[2] || '', learnTab: learnTab, meTab: meTab, puzzleTheme: puzzleTheme };
}
...
function renderWatchPage() {
  ...
  return `
    <section class="hero">
      <div class="meta">社区</div>
      <h1>公开观战与社区动态</h1>
      <p>当前社区页以公开房间与归档对局为主，支持稳定轮询刷新、棋种过滤和直达分析页。</p>
    </section>
...
function leaderboardBucket(gameType) {
  const type = gameType === 'GOMOKU' ? 'GOMOKU' : 'XIANGQI';
  const board = state.communityLeaderboard || {};
  const buckets = board.byGameType || {};
  const bucket = buckets[type];
  if (bucket && (Array.isArray(bucket.winBoard) || Array.isArray(bucket.activityBoard))) {
    return bucket;
  }
  return {
    gameType: type,
    winBoard: board.winBoard || [],
    activityBoard: board.activityBoard || []
  };
}

function leaderboardItems(gameType, boardType = 'winBoard') {
  const bucket = leaderboardBucket(gameType);
  return Array.isArray(bucket[boardType]) ? bucket[boardType] : [];
}

function renderLeaderboardRows(items, limit = 5) {
  const rows = (items || []).slice(0, limit);
  if (!rows.length) {
    return renderLeaderboardEmpty();
  }
...
function renderLeaderboardEmpty() {
  return '<div class="banner">暂无真实榜单数据，完成对局后会自动刷新。</div>';
}

function bootstrapGameStat(gameType) {
  const stats = (state.bootstrap && state.bootstrap.gameStats) || {};
  const stat = stats[gameType === 'GOMOKU' ? 'GOMOKU' : 'XIANGQI'];
  if (stat && typeof stat === 'object') {
    return stat;
  }
  return {
    gameType: gameType === 'GOMOKU' ? 'GOMOKU' : 'XIANGQI',
    totalGames: 0,
    completedGames: 0,
    trainingGames: 0,
    distinctPlayers: 0,
...
function relativeTimeLabel(isoText) {
...
function renderProfilePage() {
  return renderProfile(currentRoute());
}
...
function resolveMeSubRoute(value) {
  return ME_SUB_ROUTES.includes(value) ? value : 'overview';
}
...
function renderWatchGames(items) {
  ...
  return items.map(item => `
    <div class="move">
      ...
      <button class="ghost" data-nav="analysis/${escapeHtml(item.gameId || '')}">查看</button>
    </div>
  `).join('');
}
          ...
          <div class="boardPlayerCard ${playerActive ? 'is-active' : ''}">
            <div class="boardPlayerCardTop">
              ...
              <div class="userMeta">
                ...
                <span class="vipBadge">${viewerSide === 'RED' ? '红方' : '黑方'} · 免费练习</span>
              </div>
            </div>
    ...
    <div class="boardPlayerCard ${active ? 'is-active' : ''}" data-clock-card="${side}">
      <div class="boardPlayerCardTop">
        ...
        <div class="userMeta">
          ...
          <span class="vipBadge">${label} · 在线对局</span>
        </div>
      </div>
...
function renderProfile(route = currentRoute()) {
  const me = state.me;
  if (!me) {
  ...
  const tab = resolveMeSubRoute(route.meTab || route.id || 'overview');
  const summary = state.profile.summary || {};
  const recentGames = state.profile.recentGames || [];
  const notifications = state.profile.notifications || [];
  const achievements = state.profile.achievements || [];
  const progress = state.profile.learnProgress || state.learnProgress || { tutorialsCompleted: [], puzzlesCompleted: [] };
  const liveSignals = [state.profile.activity && state.profile.activity.room, state.profile.activity && state.profile.activity.game].filter(Boolean).length;
  const accountTag = me.id ? `账号 ${escapeHtml(String(me.id).slice(0, 8))}` : '免费账户';
  const recentActiveLabel = summary.lastGameAt ? relativeTimeLabel(summary.lastGameAt) : '最近活跃';
  return `
    <div class="profilePage">
      ${renderProfileSidebar(tab)}
        ...
        <section class="panel profileHeaderCard">
          <div class="profileUserRow">
            ...
            <div class="profileUserMeta">
              ...
              <span class="vipBadge">${accountTag} · ${recentActiveLabel}</span>
            </div>
            <button class="btn btn-red btn-small profileEditBtn" data-nav="play">开始对局</button>
          </div>
          <div class="profileStatsGrid">
            ...
            <div class="statBox">
              <strong>${summary.ratingScore || 1000}</strong>
              <span>积分</span>
            </div>
          </div>
        </section>
        
        ${renderProfileTabPanel(tab, {
          summary,
          recentGames,
          notifications,
          achievements,
          progress,
          liveSignals,
          preferences: state.profile.preferences || state.profilePreferences || null
        })}
      </div>
    </div>
  `;
...
function renderProfileSidebar(activeTab) {
  const items = [
    ['overview', '个人信息'],
    ['records', '对局记录'],
    ['study', '学习档案'],
  ...
  return `
    <aside class="panel profileSidebar">
      ${items.map(([tab, label]) => `
        <button class="profileSidebarItem ${activeTab === tab ? 'is-active' : ''}" data-nav="${tab === 'overview' ? 'me' : `me/${tab}`}">${label}</button>
      `).join('')}
    </aside>
  `;
}

function renderProfileTabPanel(tab, profile) {
  if (tab === 'records') {
    return renderProfileRecordsSection(profile.recentGames || []);
  }
  if (tab === 'study') {
    return renderProfileStudySection(profile.progress || {});
  }
  if (tab === 'inbox') {
    return renderProfileInboxSection(profile.notifications || []);
  }
  if (tab === 'achievements') {
    return renderProfileAchievementsSection(profile.achievements || []);
  }
  if (tab === 'settings') {
    return renderProfileSettingsSection(profile.preferences || {});
  }
  if (tab === 'help') {
    return renderProfileHelpSection();
  }
  return renderProfileOverviewSection(profile);
}

function renderProfileOverviewSection(profile) {
  const summary = profile.summary || {};
  const recentGames = (profile.recentGames || []).slice(0, 4);
  const notifications = (profile.notifications || []).slice(0, 3);
  return `
    <div class="profileMedalGrid">
      ...
      <div class="panel medalCard">
        ...
        <div class="medalInfo">
          <strong>最近归档</strong>
          <span>${recentGames.length} 局</span>
        </div>
      </div>
      <div class="panel medalCard">
        ...
        <div class="medalInfo">
          <strong>当前活动</strong>
          <span>${profile.liveSignals || 0} 项</span>
        </div>
      </div>
    </div>
    <div class="split profileSectionSplit">
      <section class="panel">
        <h3 class="sectionTitle">最近对局</h3>
        <div class="moves">${recentGames.length ? recentGames.map(renderProfileGameCard).join('') : '<div class="banner">还没有归档对局，去大厅或练习页开始一局。</div>'}</div>
      </section>
      <section class="panel">
        <h3 class="sectionTitle">最近通知</h3>
        <div class="moves">${notifications.length ? notifications.map(renderProfileNotificationRow).join('') : '<div class="banner">当前没有新的系统通知。</div>'}</div>
      </section>
    </div>
  `;
}

function renderProfileRecordsSection(recentGames) {
  return `
    <section class="panel">
      <h3 class="sectionTitle">对局记录</h3>
      <div class="moves">${recentGames.length ? recentGames.map(renderProfileGameCard).join('') : '<div class="banner">当前还没有归档对局。</div>'}</div>
    </section>
  `;
}

function renderProfileStudySection(progress) {
  const tutorials = (progress.tutorialsCompleted || []);
  const puzzles = (progress.puzzlesCompleted || []);
  const updated = progress.updatedAt ? relativeTimeLabel(progress.updatedAt) : '暂无更新';
  return `
    <div class="profileSettingsGrid">
      <section class="panel profileSectionCard">
        <h3 class="sectionTitle">学习档案</h3>
        <div class="profileStatList">
          <div class="statBox"><strong>${tutorials.length}</strong><span>已完成教程</span></div>
          <div class="statBox"><strong>${puzzles.length}</strong><span>已完成题目</span></div>
          <div class="statBox"><strong>${tutorials.length + puzzles.length}</strong><span>累计完成</span></div>
        </div>
        <div class="banner" style="margin-top:12px;">最近同步：${escapeHtml(updated)}</div>
      </section>
      <section class="panel profileSectionCard">
        <h3 class="sectionTitle">继续练习</h3>
        <div class="roomRow">
          <button class="btn" data-nav="learn/tutorials">继续教程</button>
          <button class="ghost" data-nav="learn/puzzles/ALL">进入题库</button>
          <button class="ghost" data-nav="learn/practice">AI 对战</button>
        </div>
        <div class="profileTagList">
          ${tutorials.slice(0, 6).map(item => `<span class="pill">教程 ${escapeHtml(item)}</span>`).join('')}
          ${puzzles.slice(0, 6).map(item => `<span class="pill">题目 ${escapeHtml(item)}</span>`).join('')}
          ${!tutorials.length && !puzzles.length ? '<div class="banner" style="margin-top:12px;">还没有学习记录，先从教程或题库开始。</div>' : ''}
        </div>
      </section>
    </div>
  `;
...
function renderProfileInboxSection(notifications) {
  return `
    <section class="panel">
      <h3 class="sectionTitle">消息通知</h3>
      <div class="moves">${notifications.length ? notifications.map(renderProfileNotificationRow).join('') : '<div class="banner">当前没有新的系统通知。</div>'}</div>
    </section>
  `;
}

function renderProfileAchievementsSection(achievements) {
  return `
    <section class="panel">
      <h3 class="sectionTitle">我的成就</h3>
      <div class="profileAchievementGrid">
        ${achievements.length ? achievements.map(renderProfileAchievementCard).join('') : '<div class="banner">当前还没有成就数据。</div>'}
      </div>
    </section>
  `;
}

function renderProfileSettingsSection(preferences) {
  const updated = preferences.updatedAt ? relativeTimeLabel(preferences.updatedAt) : '尚未同步';
  return `
    <div class="profileSettingsGrid">
      <section class="panel profileSectionCard">
        <h3 class="sectionTitle">棋桌设置</h3>
        <div class="roomRow">
          <button class="btn btn-small ${state.soundEnabled ? 'btn-red' : 'ghost'}" data-action="toggle-sound">音效：${state.soundEnabled ? '开' : '关'}</button>
          <button class="btn btn-small btn-red" data-action="toggle-theme">${state.boardTheme === 'wood' ? '古雅木纹' : '清雅水墨'}</button>
          <button class="btn btn-small ghost" data-action="flip-board">翻转：${state.boardFlipped ? '开' : '关'}</button>
        </div>
        <div class="banner" style="margin-top:12px;">最近同步：${escapeHtml(updated)}</div>
      </section>
      <section class="panel profileSectionCard">
        <h3 class="sectionTitle">影响范围</h3>
        <div class="moves">
          <div class="move"><div><strong>在线对局</strong><div class="muted">进入房间和正式对局时沿用当前偏好。</div></div></div>
          <div class="move"><div><strong>AI 练习</strong><div class="muted">练习棋桌也会沿用音效、主题和翻转设置。</div></div></div>
          <div class="move"><div><strong>当前账号</strong><div class="muted">设置保存到账号，刷新后仍保留。</div></div></div>
        </div>
      </section>
    </div>
  `;
...
function renderProfileHelpSection() {
  return `
    <div class="profileSettingsGrid">
      <section class="panel profileSectionCard">
        <h3 class="sectionTitle">帮助入口</h3>
        <div class="roomRow">
          <button class="btn" data-nav="help">使用帮助</button>
          <button class="ghost" data-nav="watch">公开观战</button>
          <button class="ghost" data-nav="community">排行榜</button>
        </div>
      </section>
      <section class="panel profileSectionCard">
        <h3 class="sectionTitle">常用操作</h3>
        <div class="moves">
          <div class="move"><div><strong>开始一局</strong><div class="muted">从大厅创建房间，或直接进入练习。</div></div><button class="ghost" data-nav="play">进入大厅</button></div>
          <div class="move"><div><strong>继续学习</strong><div class="muted">教程、残局与 AI 练习都保持免费可用。</div></div><button class="ghost" data-nav="learn">前往学习</button></div>
        </div>
      </section>
    </div>
  `;
...
function renderProfileNotificationRow(item) {
  const path = normalizeProfilePath(item.path);
  const when = item.createdAt ? relativeTimeLabel(item.createdAt) : '刚刚';
  return `
    <div class="move">
      <div>
      ...
      ${path ? `<button class="ghost" data-nav="${path}">查看</button>` : ''}
    </div>
  `;
}

function renderProfileAchievementCard(item) {
  const current = Number(item.current || 0);
  const target = Math.max(1, Number(item.target || 1));
  const percent = Math.max(0, Math.min(100, Math.round(current * 100 / target)));
  return `
    <article class="profileAchievementCard ${item.earned ? 'is-earned' : ''}">
      <div class="roomRow">
        <strong>${escapeHtml(item.title || '成就')}</strong>
        <span class="vipBadge">${item.earned ? '已达成' : `${current}/${target}`}</span>
      </div>
      <p class="muted">${escapeHtml(item.description || '')}</p>
      <div class="profileAchievementBar"><span style="width:${percent}%"></span></div>
    </article>
  `;
}
...
function renderProfileGameCard(game) {
  const updatedLabel = game.updatedAt || game.finishedAt || game.startedAt
    ? relativeTimeLabel(game.updatedAt || game.finishedAt || game.startedAt)
    : '刚刚';
  return `
    <div class="move">
      <div>
...
function renderLobbySearchResults() {
  const search = state.lobbySearch || { query: '', rooms: [], players: [], loading: false, error: '' };
  const query = String(search.query || '').trim();
  if (!query) {
    return '';
  }
  const rooms = Array.isArray(search.rooms) ? search.rooms : [];
  const players = Array.isArray(search.players) ? search.players : [];
  const hasResults = rooms.length || players.length;
  return `
    <section class="panel" style="margin-top:14px;">
      <div class="deskSectionHeader">
        <h3>搜索结果</h3>
        <span class="muted">关键词：${escapeHtml(query)}</span>
      </div>
      ${search.loading ? '<div class="banner">搜索中...</div>' : ''}
      ${search.error ? `<div class="banner">${escapeHtml(search.error)}</div>` : ''}
      ${!search.loading && !search.error && !hasResults ? '<div class="banner">没有匹配的公开房间或玩家。</div>' : ''}
      ${rooms.length ? `
        <div class="stack" data-lobby-search-rooms>
          ${rooms.map(room => {
            const target = room.gameId ? `game/${room.gameId}` : `room/${room.roomId}`;
            const subtitle = `${room.hostUsername || '-'}${room.guestUsername ? ` vs ${room.guestUsername}` : ' · 等待加入'}`;
            return `
              <button class="move" data-nav="${target}">
                <div>
                  <strong>${escapeHtml(room.roomCode || '-')} · ${escapeHtml(room.gameType || '-')}</strong>
      ...
      ` : ''}
      ${players.length ? `
        <div class="stack" style="margin-top:${rooms.length ? '12px' : '0'};" data-lobby-search-players>
          ${players.map(player => `
            <button class="move" data-nav="community">
              <div>
                <strong>${escapeHtml(player.username || '-')}</strong>
                <div class="muted">匹配到玩家名，进入排行榜查看相关信息</div>
              </div>
              <span class="pill">玩家</span>
            </button>
          `).join('')}
        </div>
      ` : ''}
...
function bindCommon(route) {
  ...
  on('[data-action="quick-match-xiangqi"]', () => quickStartPublicMatch('XIANGQI', 300));
  on('[data-action="quick-match-gomoku"]', () => quickStartPublicMatch('GOMOKU', 300));
  ...
  document.querySelectorAll('[data-action="toggle-theme"]').forEach(el => {
    ...
    el.addEventListener('click', async () => {
      ...
      render();
      if (state.me) {
        await saveProfilePreferencesPatch({ boardTheme: state.boardTheme });
      }
    });
  });

  document.querySelectorAll('[data-leaderboard-game-type]').forEach(el => {
    if (el.dataset.boundLeaderboardGameType === '1') return;
    el.dataset.boundLeaderboardGameType = '1';
    el.addEventListener('click', () => {
      const nextType = el.getAttribute('data-leaderboard-game-type');
      if (nextType !== 'XIANGQI' && nextType !== 'GOMOKU') {
        return;
      }
      if (state.leaderboardGameType === nextType) {
        return;
      }
      state.leaderboardGameType = nextType;
      render();
    });
  });

  document.querySelectorAll('[data-community-game-type]').forEach(el => {
    if (el.dataset.boundCommunityGameType === '1') return;
    el.dataset.boundCommunityGameType = '1';
    el.addEventListener('click', () => {
      const nextType = el.getAttribute('data-community-game-type');
      if (nextType !== 'XIANGQI' && nextType !== 'GOMOKU') {
        return;
      }
      if (state.leaderboardGameType === nextType) {
  ...
  document.querySelectorAll('[data-action="flip-board"]').forEach(el => {
    ...
    el.addEventListener('click', async () => {
      ...
      render();
      if (state.me) {
        await saveProfilePreferencesPatch({ boardFlipped: state.boardFlipped });
      }
    });
  });

  document.querySelectorAll('[data-lobby-search-input]').forEach(el => {
    if (el.dataset.boundLobbySearch === '1') return;
    el.dataset.boundLobbySearch = '1';
    el.addEventListener('input', () => {
      const query = el.value || '';
      state.lobbySearch = {
        ...(state.lobbySearch || { rooms: [], players: [], loading: false, error: '' }),
        query: query
      };
      if (state.lobbySearchTimer) {
        window.clearTimeout(state.lobbySearchTimer);
    ...
    el.addEventListener('keydown', event => {
      ...
      loadLobbySearch(el.value || '', true);
    });
  });

  document.querySelectorAll('[data-join-code-input]').forEach(el => {
    if (el.dataset.boundJoinCodeInput === '1') return;
    el.dataset.boundJoinCodeInput = '1';
    el.addEventListener('input', () => {
      state.joinCodeDraft = String(el.value || '').toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 8);
      if (el.value !== state.joinCodeDraft) {
        el.value = state.joinCodeDraft;
      }
    });
    el.addEventListener('keydown', event => {
      if (event.key !== 'Enter') {
...
async function quickStartPublicMatch(gameType = 'XIANGQI', initialTimeSeconds = 300) {
  ...
  try {
    ...
    state.room = room;
    state.game = result.game ? applyServerGameSnapshot(result.game) : null;
    state.lobby = null;
    state.status = result.matched ? '已匹配到对手，在线对局已准备就绪。' : '已创建公开候场房，等待棋友加入。';
    await refreshBootstrapAndProfile();
    if (state.game && state.game.gameId) {
      navTo(`game/${state.game.gameId}`);
    } else {
      navTo(`room/${room.roomId}`);
    }
  } catch (error) {
...
async function joinByCode() {
  try {
    ...
    const roomCode = ((input && input.value) || state.joinCodeDraft || '').trim().toUpperCase();
    state.joinCodeDraft = roomCode;
    if (!roomCode) {
      state.status = '请输入邀请码。';
      render();
      return;
...
async function loadProfile() {
  state.profile = await fetchJson(`${API_BASE}/profile/dashboard`).catch(() => null);
  if (state.profile && state.profile.preferences) {
    applyProfilePreferences(state.profile.preferences);
  }
  render();
}
...
async function refreshBootstrapAndProfile() {
  await loadBootstrap();
  if (state.me) {
    state.profile = await fetchJson(`${API_BASE}/profile/dashboard`).catch(() => state.profile);
    if (state.profile && state.profile.preferences) {
      applyProfilePreferences(state.profile.preferences);
    } else {
      await loadProfilePreferences();
...
function applyProfilePreferences(preferences) {
  state.profilePreferences = preferences || null;
  if (!preferences) {
    return;
  }
  state.soundEnabled = preferences.soundEnabled !== false;
  persistOnlineSoundEnabled(state.soundEnabled);
  state.boardTheme = preferences.boardTheme === 'ink' ? 'ink' : 'wood';
  state.boardFlipped = !!preferences.boardFlipped;
}

async function saveProfilePreferencesPatch(patch) {
  if (!state.me) {
    return null;
  }
...
function toggleOnlineSound() {
  ...
  render();
  if (state.me) {
    saveProfilePreferencesPatch({ soundEnabled: state.soundEnabled });
  }
}
...
function renderHomePageGuofeng() {
  ...
  const leaderboardType = state.leaderboardGameType === 'GOMOKU' ? 'GOMOKU' : 'XIANGQI';
  const leaderboardItemsForHome = leaderboardItems(leaderboardType, 'winBoard');
  const recentGames = (b.recentGames || []).slice(0, 4);
  return `
    <div class="deskHome">
      <section class="deskHero panel">
        ...
        <div class="deskHeroCopy">
          ...
          <h1>落子之间，自有风雅</h1>
          <p>在线象棋与五子棋同台并行，保留现有对局、AI 练习、复盘与排行榜能力，只把网页端换成更完整的桌面国风界面。</p>
          ...
          <div class="deskHeroFacts">
            <span class="pill">活动房间 ${b.activeRooms || 0}</span>
            <span class="pill">棋友数量 ${b.totalUsers || 0}</span>
            <span class="pill">已归档对局 ${b.totalGames || 0}</span>
          </div>
        </div>
      ...
      <div class="deskQuickGrid">
        <button class="deskQuickItem" data-action="quick-match-xiangqi">
          <span class="icon">⚡</span>
        ...
        <button class="deskQuickItem" data-nav="learn">
          <span class="icon">📖</span>
          <div class="text"><strong>练习入口</strong><span>题库教程 随时进入</span></div>
        </button>
        <button class="deskQuickItem" data-nav="me">
          <span class="icon">👤</span>
          <div class="text"><strong>个人中心</strong><span>战绩总览 常用入口</span></div>
        </button>
      </div>
      ...
      <div class="deskHomeThreeCol">
        <section class="panel col-left">
          ...
          <div class="quickPortalGrid">
            <button class="quickPortalCard" data-action="quick-match-xiangqi">
              <strong>快速匹配</strong>
              <span>公开房间候场</span>
            </button>
            <button class="quickPortalCard" data-action="create-room-xiangqi">
              <strong>智能约局</strong>
              <span>创建私密棋房</span>
            </button>
            <button class="quickPortalCard" data-nav="watch">
              <strong>公开观战</strong>
              <span>进入社区观战</span>
            </button>
            <button class="quickPortalCard" data-nav="learn">
              <strong>残局练习</strong>
              <span>题库与教程入口</span>
            </button>
          </div>
        ...
        <section class="panel col-mid">
          <div class="deskSectionHeader">
            <h3>对局实况</h3>
            <button class="ghost" data-nav="play">进入大厅</button>
          </div>
          <div class="activityList">
            ${recentGames.map(game => `
              <button class="activityRow" data-nav="analysis/${game.gameId}">
                <div>
                  <strong>${game.gameType === 'XIANGQI' ? '象棋房' : '五子棋房'}</strong>
                  <div class="muted">${escapeHtml(game.firstUsername || '-')} vs ${escapeHtml(game.secondUsername || '-')}</div>
                </div>
                <span class="pill">${escapeHtml(game.resultText || game.status || '已归档')}</span>
              </button>
            `).join('') || '<div class="banner">当前暂无实况数据，可先进入大厅创建对局。</div>'}
          </div>
        </section>
        ...
        <section class="panel col-right">
          <div class="deskSectionHeader">
            <h3>排行榜</h3>
            <div class="tabHeader">
              <button class="tabItem ${leaderboardType === 'XIANGQI' ? 'active' : ''}" data-leaderboard-game-type="XIANGQI">象棋</button>
              <button class="tabItem ${leaderboardType === 'GOMOKU' ? 'active' : ''}" data-leaderboard-game-type="GOMOKU">五子棋</button>
            </div>
          </div>
          <div class="deskRankList">
            ${renderLeaderboardRows(leaderboardItemsForHome, 5)}
          </div>
          <button class="ghost btn-block" data-nav="community">查看全部</button>
        </section>
      </div>
    </div>
  `;
...
function renderPlayLobbyDesk() {
  ...
  const leaderboardType = state.leaderboardGameType === 'GOMOKU' ? 'GOMOKU' : 'XIANGQI';
  const leaderboardItemsForLobby = leaderboardItems(leaderboardType, 'winBoard');
  const searchQuery = escapeHtml((state.lobbySearch && state.lobbySearch.query) || '');
  const joinCodeValue = escapeHtml(state.joinCodeDraft || '');
  return `
    <div class="deskLobby">
      <aside class="panel deskSidebar">
        <div class="sidebarUser">
          ...
          <div class="userMeta">
            ...
            <span class="vipBadge">免费开放 · 随时开局</span>
          </div>
        </div>
        <button class="deskSidebarItem" data-nav="home">首页</button>
        <button class="deskSidebarItem is-active" data-nav="play">对弈大厅</button>
      ...
      <section class="deskLobbyMain">
        <div class="panel deskLobbySearch">
          <div class="searchBar">
            ...
            <input id="lobbySearchQuery" data-lobby-search-input type="text" placeholder="搜索房间、玩家、房间号..." value="${searchQuery}" />
          ...
          <div class="deskLobbyTabs">
            ...
            <button class="pill" data-nav="learn">练习</button>
            <button class="pill" data-action="create-room-xiangqi">好友房</button>
          </div>
        </div>
        ${renderLobbySearchResults()}
        <div class="deskLobbyCards">
          ...
          <div class="panel deskModePanel deskModePanel--green" data-nav="play/gomoku">
            <div class="deskModePanelCopy">
              ...
              <h3>五子棋</h3>
              <p>五子连珠，乐趣其中</p>
              <button class="btn" data-action="quick-match-gomoku">进入大厅</button>
            </div>
          ...
          <div class="recentList">
            ${recentGames.map(game => `
              <div class="recentRow" data-nav="analysis/${game.gameId}">
                ...
                <span class="gameTime">${relativeTimeLabel(game.updatedAt || game.finishedAt || game.startedAt)}</span>
              </div>
      ...
      <aside class="panel deskLobbyAside">
        ...
        <div class="lobbyActionGrid">
          ...
          <button class="actionBtn actionBtn--join" data-action="create-room-gomoku">
            <span class="actionIcon">◎</span>
            <div class="actionText">
              <strong>创建五子棋房</strong>
              <span>好友对局 随时开桌</span>
            </div>
          </button>
        </div>
        <div class="field" style="margin-top:18px;">
          <label>邀请码</label>
          <input id="joinCode" data-join-code-input type="text" maxlength="8" placeholder="输入 8 位房间码" value="${joinCodeValue}" />
        </div>
        <button class="btn btn-block" style="margin-top:12px;" data-action="join-by-code">通过邀请码加入</button>
        <div class="deskSectionHeader">
          <h3>排行榜</h3>
          <div class="tabHeader">
            <button class="tabItem ${leaderboardType === 'XIANGQI' ? 'active' : ''}" data-leaderboard-game-type="XIANGQI">象棋</button>
        ...
        <div class="deskRankList">
          ${renderLeaderboardRows(leaderboardItemsForLobby, 5)}
        </div>
    ...
    <div class="modeSelectPage xiangqiTheme">
      ...
      <div class="modeDetailCard">
        ...
        <div class="quickActions">
          <div class="actionBox" data-action="quick-match-xiangqi" style="cursor:pointer;">
            ⚡
            <span>实时匹配</span>
          </div>
...
function renderPlayGomoku() {
  const stat = bootstrapGameStat('GOMOKU');
  return `
    <div class="modeSelectPage gomokuTheme">
      ...
      <div class="modeDetailCard">
        ...
        <div class="statsGrid">
          ...
          <div class="statBox">
            <strong>${stat.completedGames || 0}局</strong>
            <span>已归档</span>
          </div>
          <div class="statBox">
            <strong>${stat.distinctPlayers || 0}人</strong>
            <span>参与</span>
          </div>
        </div>
      </div>
