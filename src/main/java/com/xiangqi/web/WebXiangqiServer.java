...
public class WebXiangqiServer {
    ...
    private static final class Session {
        ...
        return String.join("\n",
            ...
            "function drawStatic(){cctx.clearRect(0,0,BASE_W,BASE_H);const bx=MARGIN-30,by=MARGIN-30,bw=8*CELL+60,bh=9*CELL+60;if(boardTexReady){cctx.drawImage(boardTex,bx,by,bw,bh);cctx.fillStyle='rgba(22,35,94,.30)';cctx.fillRect(MARGIN-2,MARGIN-2,8*CELL+4,9*CELL+4);}else{const g=cctx.createLinearGradient(0,0,0,BASE_H);g.addColorStop(0,'#24356c');g.addColorStop(1,'#1f2c5e');cctx.fillStyle=g;cctx.fillRect(MARGIN-26,MARGIN-26,8*CELL+52,9*CELL+52);}cctx.strokeStyle='#b98a52';cctx.lineWidth=4;cctx.strokeRect(bx,by,bw,bh);cctx.strokeStyle='#d5b26a';cctx.lineWidth=2;for(let r=0;r<10;r++){const y=MARGIN+r*CELL;cctx.beginPath();cctx.moveTo(MARGIN,y);cctx.lineTo(MARGIN+8*CELL,y);cctx.stroke();}for(let c=0;c<9;c++){const x=MARGIN+c*CELL;cctx.beginPath();if(c===0||c===8){cctx.moveTo(x,MARGIN);cctx.lineTo(x,MARGIN+9*CELL);}else{cctx.moveTo(x,MARGIN);cctx.lineTo(x,MARGIN+4*CELL);cctx.moveTo(x,MARGIN+5*CELL);cctx.lineTo(x,MARGIN+9*CELL);}cctx.stroke();}cctx.beginPath();cctx.moveTo(MARGIN+3*CELL,MARGIN);cctx.lineTo(MARGIN+5*CELL,MARGIN+2*CELL);cctx.moveTo(MARGIN+5*CELL,MARGIN);cctx.lineTo(MARGIN+3*CELL,MARGIN+2*CELL);cctx.moveTo(MARGIN+3*CELL,MARGIN+7*CELL);cctx.lineTo(MARGIN+5*CELL,MARGIN+9*CELL);cctx.moveTo(MARGIN+5*CELL,MARGIN+7*CELL);cctx.lineTo(MARGIN+3*CELL,MARGIN+9*CELL);cctx.stroke();cctx.fillStyle='rgba(44,63,130,.82)';cctx.fillRect(MARGIN+1,MARGIN+4*CELL+1,8*CELL-2,CELL-2);cctx.strokeStyle='rgba(213,178,106,.35)';cctx.beginPath();cctx.moveTo(MARGIN,MARGIN+4*CELL);cctx.lineTo(MARGIN+8*CELL,MARGIN+4*CELL);cctx.moveTo(MARGIN,MARGIN+5*CELL);cctx.lineTo(MARGIN+8*CELL,MARGIN+5*CELL);cctx.stroke();cctx.font='bold 48px KaiTi';cctx.fillStyle='#d8b574';cctx.fillText('楚 河',MARGIN+CELL-8,MARGIN+4*CELL+52);cctx.fillText('汉 界',MARGIN+5*CELL+8,MARGIN+4*CELL+52);}",
