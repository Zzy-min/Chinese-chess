...
public class WebXiangqiServer {
    ...
    private static final class Session {
        ...
        return String.join("\n",
            ...
            "    .wrap{width:min(1320px,98vw);height:calc(100vh - 24px);background:linear-gradient(180deg,rgba(255,248,236,.94),rgba(245,226,196,.93));border:1px solid #d9bd93;border-radius:14px;box-shadow:0 18px 38px rgba(16,11,7,.35);padding:12px;box-sizing:border-box;position:relative;}",
            ...
            "    .boardCard{background:linear-gradient(180deg,#efe5d2,#ead8bb);border:1px solid #d6b98f;border-radius:10px;padding:8px;display:flex;align-items:center;justify-content:center;overflow:hidden;position:relative;}",
            ...
            "function drawStatic(){cctx.clearRect(0,0,BASE_W,BASE_H);const bx=MARGIN-34,by=MARGIN-34,bw=8*CELL+68,bh=9*CELL+68;if(boardTexReady){cctx.drawImage(boardTex,bx,by,bw,bh);}else{const g=cctx.createLinearGradient(0,0,0,BASE_H);g.addColorStop(0,'#2c3f88');g.addColorStop(1,'#1f2f66');cctx.fillStyle=g;cctx.fillRect(bx,by,bw,bh);}cctx.fillStyle='rgba(20,33,88,.34)';cctx.fillRect(MARGIN-2,MARGIN-2,8*CELL+4,9*CELL+4);cctx.strokeStyle='#8d5d2f';cctx.lineWidth=8;cctx.strokeRect(bx-6,by-6,bw+12,bh+12);cctx.strokeStyle='#d8b36b';cctx.lineWidth=3;cctx.strokeRect(bx,by,bw,bh);const topY=by-20;const botY=by+bh+20;cctx.strokeStyle='rgba(108,64,28,.9)';cctx.lineWidth=3;cctx.beginPath();cctx.moveTo(bx+30,topY);cctx.bezierCurveTo(bx+bw*0.32,topY-16,bx+bw*0.68,topY-16,bx+bw-30,topY);cctx.moveTo(bx+30,botY);cctx.bezierCurveTo(bx+bw*0.32,botY+16,bx+bw*0.68,botY+16,bx+bw-30,botY);cctx.stroke();cctx.strokeStyle='rgba(213,178,106,.86)';cctx.lineWidth=2;for(let r=0;r<10;r++){const y=MARGIN+r*CELL;cctx.beginPath();cctx.moveTo(MARGIN,y);cctx.lineTo(MARGIN+8*CELL,y);cctx.stroke();}for(let c=0;c<9;c++){const x=MARGIN+c*CELL;cctx.beginPath();if(c===0||c===8){cctx.moveTo(x,MARGIN);cctx.lineTo(x,MARGIN+9*CELL);}else{cctx.moveTo(x,MARGIN);cctx.lineTo(x,MARGIN+4*CELL);cctx.moveTo(x,MARGIN+5*CELL);cctx.lineTo(x,MARGIN+9*CELL);}cctx.stroke();}cctx.beginPath();cctx.moveTo(MARGIN+3*CELL,MARGIN);cctx.lineTo(MARGIN+5*CELL,MARGIN+2*CELL);cctx.moveTo(MARGIN+5*CELL,MARGIN);cctx.lineTo(MARGIN+3*CELL,MARGIN+2*CELL);cctx.moveTo(MARGIN+3*CELL,MARGIN+7*CELL);cctx.lineTo(MARGIN+5*CELL,MARGIN+9*CELL);cctx.moveTo(MARGIN+5*CELL,MARGIN+7*CELL);cctx.lineTo(MARGIN+3*CELL,MARGIN+9*CELL);cctx.stroke();cctx.fillStyle='rgba(35,54,124,.80)';cctx.fillRect(MARGIN+1,MARGIN+4*CELL+1,8*CELL-2,CELL-2);cctx.strokeStyle='rgba(213,178,106,.44)';cctx.beginPath();cctx.moveTo(MARGIN,MARGIN+4*CELL);cctx.lineTo(MARGIN+8*CELL,MARGIN+4*CELL);cctx.moveTo(MARGIN,MARGIN+5*CELL);cctx.lineTo(MARGIN+8*CELL,MARGIN+5*CELL);cctx.stroke();cctx.font='bold 46px KaiTi';cctx.fillStyle='#d8b574';cctx.fillText('楚 河',MARGIN+CELL-10,MARGIN+4*CELL+46);cctx.fillText('汉 界',MARGIN+5*CELL+8,MARGIN+4*CELL+46);}",
            ...
            "function drawPieceDisc(x,y,name,color){ctx.fillStyle='rgba(13,8,0,.24)';ctx.beginPath();ctx.ellipse(x+2,y+3,R*0.98,R*0.82,0,0,Math.PI*2);ctx.fill();const g=ctx.createRadialGradient(x-9,y-10,4,x,y,R);if(color==='RED'){g.addColorStop(0,'#fff7ec');g.addColorStop(0.62,'#efd8bd');g.addColorStop(1,'#d1ad86');}else{g.addColorStop(0,'#ffffff');g.addColorStop(0.62,'#ebe7df');g.addColorStop(1,'#c7c2b8');}ctx.fillStyle=g;ctx.beginPath();ctx.arc(x,y,R,0,Math.PI*2);ctx.fill();ctx.strokeStyle='rgba(88,58,29,.92)';ctx.lineWidth=2.2;ctx.beginPath();ctx.arc(x,y,R,0,Math.PI*2);ctx.stroke();ctx.strokeStyle=(color==='RED')?'#d3524b':'#2d2d2d';ctx.lineWidth=2.4;ctx.beginPath();ctx.arc(x,y,R-3,0,Math.PI*2);ctx.stroke();ctx.strokeStyle='rgba(229,207,160,.9)';ctx.lineWidth=1.1;ctx.beginPath();ctx.arc(x,y,R-6,0,Math.PI*2);ctx.stroke();ctx.fillStyle='rgba(255,255,255,.22)';ctx.beginPath();ctx.arc(x-8,y-10,7,0,Math.PI*2);ctx.fill();ctx.font='bold 34px KaiTi';ctx.fillStyle=(color==='RED')?'#c6403c':'#1f1f1f';const w=ctx.measureText(name).width;ctx.fillText(name,x-w/2,y+12);}function drawPieces(){for(let r=0;r<10;r++){for(let c=0;c<9;c++){const p=state.board[r][c];if(!p)continue;const [x,y]=pos(r,c);drawPieceDisc(x,y,p.name,p.color);}}}",
